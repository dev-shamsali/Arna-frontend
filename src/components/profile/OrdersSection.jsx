...
import { Package, ChevronDown, ChevronUp, MapPin, Truck, CheckCircle, Clock, Box, XCircle, ShoppingBag, ArrowRight } from "lucide-react";
...
const mapShipmentStatus = (status) => {
  const mapping = {
    ...
    delivered: "Delivered",
    failed: "Cancelled", // or Failed if you want separate UI
  };
...
function TrackingTimeline({ status }) {
  const currentStep = TRACKING_STEPS.findIndex(
  ...
  if (isCancelled) {
    ...
    const bg = isCancelled ? "bg-red-50" : "bg-gray-50";
    const border = isCancelled ? "border-red-100" : "border-gray-100";
    return (
        ...
        {TRACKING_STEPS.map((step, idx) => {
          ...
          return (
            <div key={step.key} className="flex-1 flex flex-col items-center relative">
              ...
              {/* Icon circle */}
              <div
                className={`relative z-10 w-8 h-8 rounded-full flex items-center justify-center border-2 transition-all duration-500 ${isCompleted
                  ? "bg-[var(--arna-accent)] border-[var(--arna-accent)] shadow-md shadow-emerald-200"
                  : "bg-white border-gray-200"
                  } ${isCurrent ? "ring-4 ring-[var(--arna-accent)]/20" : ""}`}
              >
                <StepIcon
                  className={`w-3.5 h-3.5 transition-colors duration-300 ${isCompleted ? "text-white" : "text-gray-300"
                    }`}
                />
              </div>
              ...
              <div className="mt-2 text-center px-1">
                <p
                  className={`text-[10px] leading-tight font-semibold tracking-wide transition-colors duration-300 ${isCompleted ? "text-[var(--arna-accent)]" : "text-gray-300"
                    } ${isCurrent ? "text-[var(--arna-accent)]" : ""}`}
                >
                  {step.label}
                </p>
                <p
                  className={`text-[9px] leading-tight mt-0.5 hidden sm:block transition-colors duration-300 ${isCompleted ? "text-gray-500" : "text-gray-300"
                    }`}
                >
                  {step.description}
                </p>
...
function OrderCard({ order, router }) {
  ...
  return (
    <div className="group rounded-2xl border border-gray-100 bg-white shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-wrap sm:flex-nowrap">
            ...
            <button
              ...
              className={`inline-flex cursor-pointer items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-semibold border transition-all duration-200 ${isTrackingOpen
      ...
      {/* Tracking Dropdown */}
      <div
        className={`overflow-hidden transition-all duration-500 ease-in-out ${isTrackingOpen ? "max-h-64 opacity-100" : "max-h-0 opacity-0"
          }`}
      >
